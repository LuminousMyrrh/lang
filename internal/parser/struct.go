...
func (p *Parser) parseStructDef() *StructDefNode {
	...
	for p.currentToken() != nil && p.currentToken().TType != token.RCurly {
	...
	if p.currentToken() == nil || p.currentToken().TType != token.RCurly {
...
func (p *Parser) parseStructMethodCall(node Node) *StructMethodCall {
	...
	for p.currentToken() != nil && p.currentToken().TType == token.Dot {
		p.advance() // skip '.'
		if p.currentToken() == nil || p.currentToken().TType != token.Identifier {
		...
		if p.currentToken() != nil && p.currentToken().TType == token.LParen {
			p.advance() // skip '('
			var args []Node
			for p.currentToken() != nil && p.currentToken().TType != token.RParen {
				if p.currentToken().TType == token.Comma {
					p.advance()
					continue
				}
...
func (p *Parser) parseStructField() *StructField {
	var isPub bool = true
	if p.currentToken().TType == token.Private {
		isPub = false
	}
	p.advance()
	nameTok := p.currentToken()
	name := nameTok.Lexeme
	p.advance()
	if p.currentToken().TType == token.Assign {
		p.advance()
		value := p.parseValue()
		return &StructField{
			Name: name,
...
func (p *Parser) parseStructMethodDef() *StructMethodDef {
	isPub := true
	if p.currentToken() != nil && p.currentToken().TType == token.Private {
		isPub = false
	}
	p.advance()
	...
	p.advance()
	if p.currentToken() == nil || p.currentToken().TType != token.RightArrow {
		p.genError("Expected '->' after struct name")
		return nil
	}
	p.advance()
	...
	p.advance() // skip ')'
	if p.currentToken() == nil || p.currentToken().TType != token.LCurly {
...
func (p *Parser) parseStructInit() *StructInitNode {
	...
	for p.currentToken() != nil &&
		p.currentToken().TType != token.RCurly {
		if p.currentToken().TType == token.Comma {
			p.advance()
			continue
		}
		...
		p.advance()
		if p.currentToken().TType != token.Colon {
	...
	if p.currentToken().TType != token.RCurly {
